...
class AccountMove(models.Model):
    ...
    journal_entry_ids = fields.Many2many(
        comodel_name="account.move", compute="_compute_journal_entry_ids", store=False
    )

    def _compute_journal_entry_ids(self):
        # TODO: Maybe add a template_id?
        for this in self:
            this.journal_entry_ids = self.search(
                [("ref", "=", this.name), ("id", "!=", this.id)]
            )
    ...
    def action_post(self):
        ...
        self.journal_entry_ids.journal_item_ids.unlink()
    ...
    def _create_journal_entry_item(self, quantity, journal, item):
        ...
        # will be valid, in the account.move.auto model
